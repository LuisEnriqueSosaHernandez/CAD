...
int main(int argc,char * argv[]){
  ...
  if (fclose(fichero)!=0)
  {
	...
	return -1;
		...
		MPI_Send(&matriz2[i],tam,MPI_LONG_LONG_INT,contProcesos,etiqueta,MPI_COMM_WORLD);
		i++;
  ...
  if (fclose(fichero)!=0)
  {
	...
	break;
		...
		MPI_Recv(vector2,tam,MPI_LONG_LONG_INT,0,etiqueta,MPI_COMM_WORLD,&estado);
		//printf("Recibido en procesador %d desde %d\n",rango,estado.MPI_SOURCE);
		for(j=0;j<tam;j++){
			matrizTemporal[i][j]=Sumando(vector[j],vector2[j]);			
		}
	}
		for(i=0;i<vectorTrabajo[rango-1];i++){
